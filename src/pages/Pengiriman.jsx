        ...
        <React.Fragment key={step.name}>
          <div className="flex flex-col items-center">
            <div className={`flex items-center justify-center w-12 h-12 rounded-full ${
              index <= currentIndex ? 'bg-theme-primary text-white' : 'bg-slate-200 text-slate-500'
            }`}>
              <step.icon size={24} />
            </div>
            <p className={`mt-2 text-sm font-medium ${
              index <= currentIndex ? 'text-theme-primary-dark' : 'text-text-muted'
            }`}>
              {step.name}
            </p>
          </div>
          {index < steps.length - 1 && (
            <div className={`flex-1 h-1 ${
              index < currentIndex ? 'bg-theme-primary' : 'bg-slate-200'
            }`} />
          )}
        </React.Fragment>
      ))}
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-content-bg shadow-md rounded-lg p-6">
        <h2 className="text-xl font-semibold text-text-main mb-4">Lacak Pesanan</h2>
        <div className="flex space-x-4">
          ...
          <button className="bg-theme-primary hover:bg-theme-primary-dark text-white font-medium py-2 px-6 rounded-lg">
            Lacak
          </button>
        </div>
        
        <div className="mt-6 border-t border-border-main pt-6">
          <h3 className="text-lg font-semibold">Hasil: {trackingInfo.resi}</h3>
          <p className="text-text-muted">Estimasi Tiba: {trackingInfo.estimasi}</p>
          <div className="mt-4">
      ...
      <div className="bg-content-bg shadow-md rounded-lg overflow-hidden">
        <h2 className="text-xl font-semibold text-text-main p-6">Daftar Pengiriman Aktif</h2>
              ...
              <tr key={item.id} className="hover:bg-slate-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                    item.status === 'tiba' ? 'bg-green-100 text-green-800' : 
                    item.status === 'perjalanan' ? 'bg-blue-100 text-blue-800' : 'bg-yellow-100 text-yellow-800'
                  }`}>
                    {item.status}
                  </span>
                </td>
